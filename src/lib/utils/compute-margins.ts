...
type MarginType = StandardProperties['marginTop'];
...
/**
 * Given a `margin` shorthand value, parse it into its 4 separate longhand values
 */
...
export function computeMargins(
	...
	let result: MarginResult = {
		marginTop: defaultProperties?.marginTop ?? undefined,
		marginRight: defaultProperties?.marginRight ?? undefined,
		marginBottom: defaultProperties?.marginBottom ?? undefined,
		marginLeft: defaultProperties?.marginLeft ?? undefined
	};
